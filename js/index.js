...
function lookInit() {
    ...
    $("#divselect4 ul li a").click(function(){//当lookbook select opation被点击，显示对应slider
        ...
        var brandIconBoxA=$('#lookbook .brandIconBox div>a');

        brandIconBoxA.each(function(){
            if ($(this).attr('href')!="javascript:get(0)") {
                var href=$(this).attr('href');
              // _href=href.substr(href.indexOf('#'),href.indexOf('&')-href.indexOf('#'));
               if (value==1) {
                href=href.substr(0,href.indexOf('#'))+"#lookbook&";
               }else{
                href=href.substr(0,href.indexOf('#'))+"#product&";
            }
                $(this).attr('href',href);
            };
        })
...
var navListText=['首页','品牌','产品','大片','关于我们','视频','加盟','店铺','联系方式'];
var navA=$('.nav li a');
